...
### Crate layout
- `crates/compiled_swc_plugin/`: SWC plugin entry with `Cargo.toml`, exposing a `transform(program, config) -> (Program, StyleArtifacts)` API and hosting the supporting modules:
  - `src/css.rs`: Rust port of the CSS transformation pipeline mirroring `@compiled/css`, including atomicification, sorting, and hashing helpers.
  - `src/eval.rs`: shared static evaluation utilities (resolver integration, AST traversal, cache) to keep the plugin modular.
  - `src/hash.rs`: murmurhash2 GC implementation shared across the transform pipeline.【F:packages/utils/src/hash.ts†L1-L39】
