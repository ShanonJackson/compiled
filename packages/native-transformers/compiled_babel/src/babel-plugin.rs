...
use crate::types::{
    ...
    TransformFile, TransformMetadata, TransformState,
};
        ...
        match program {
            ...
            Program::Script(script) => {
                self.process_jsx_pragmas();
                {
                    ...
                    script.visit_mut_with(&mut guard);
                }
                script.visit_mut_children_with(self);
                {
                    let mut state = self.state.borrow_mut();
                    preserve_leading_comments(&script.body, &mut state);
                }
...
impl JsxFunctionGuard {
    ...
    fn handle_expr(&self, expr: &Expr) {
        ...
        let meta = self.meta.with_own_span(Some(expr.span()));
        let error = build_code_frame_error(&message, Some(expr.span()), &meta);
        panic!("{error}");
    }
}
...
impl VisitMut for JsxFunctionGuard {
    ...
    fn visit_mut_expr(&mut self, expr: &mut Expr) {
        ...
        match expr {
            Expr::Call(_) => self.handle_expr(expr),
            _ => {}
        }
    }
}
