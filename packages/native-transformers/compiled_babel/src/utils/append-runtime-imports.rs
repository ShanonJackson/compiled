...
pub fn append_runtime_imports(program: &mut Program, state: &mut TransformState) {
...
mod tests {
    ...
    fn parse_program(code: &str) -> Program {
        ...
        Program::Module(parser.parse_module().expect("parse module"))
    }

    fn local_names(program: &Program) -> Vec<String> {
        let Program::Module(module) = program else {
            return Vec::new();
        };
    ...
    fn inserts_runtime_import_when_missing() {
        let mut program = parse_program("const value = 1;");
        ...
        match &module.body[0] {
            ModuleItem::ModuleDecl(ModuleDecl::Import(import)) => {
                ...
                assert_eq!(local_names(&program), vec!["ax", "ix", "CC", "CS"]);
            }
    ...
    fn appends_missing_specifiers_to_existing_import() {
        ...
        assert_eq!(local_names(&program), vec!["ix", "ax", "CC", "CS"]);
    }
    ...
    fn respects_compression_map() {
        ...
        assert_eq!(local_names(&program), vec!["ac", "ix", "CC", "CS"]);
    }
