    ...
    for component in values {
        match component {
            ...
            ComponentValue::KeyframeBlock(block) => {
                increase_simple_block_specificity(&mut block.block)
            }
            _ => {}
        }
    }
    ...
    compound
        ...
        .any(|selector| match selector {
            ...
            SubclassSelector::PseudoElement(pseudo) => {
                pseudo_element_contains_compiled_class(pseudo)
            }
            _ => false,
        })
}
            ...
            children.iter().any(|child| match child {
                ...
                PseudoClassSelectorChildren::ForgivingSelectorList(list) => {
                    list.children.iter().any(|item| match item {
                        ...
                        swc_core::css::ast::ForgivingComplexSelector::ListOfComponentValues(_) => {
                            false
                        }
                    })
                }
                PseudoClassSelectorChildren::ForgivingRelativeSelectorList(list) => {
                    list.children.iter().any(|item| match item {
                        swc_core::css::ast::ForgivingRelativeSelector::RelativeSelector(
                            relative,
                        ) => complex_selector_contains_compiled_class(&relative.selector),
                        swc_core::css::ast::ForgivingRelativeSelector::ListOfComponentValues(_) => {
                            false
                        }
                    })
                }
            ...
            match child {
                ...
                PseudoClassSelectorChildren::ForgivingSelectorList(list) => {
                    for item in &mut list.children {
                        if let swc_core::css::ast::ForgivingComplexSelector::ComplexSelector(
                            selector,
                        ) = item
                        {
                            increase_complex_selector_specificity(selector);
                ...
                PseudoClassSelectorChildren::ForgivingRelativeSelectorList(list) => {
                    for item in &mut list.children {
                        if let swc_core::css::ast::ForgivingRelativeSelector::RelativeSelector(
                            relative,
                        ) = item
                        {
                            increase_complex_selector_specificity(&mut relative.selector);
