    ...
    fn run(&self, stylesheet: &mut Stylesheet, ctx: &mut TransformContext<'_>) {
        ...
        for rule in std::mem::take(&mut stylesheet.rules) {
            match rule {
                ...
                Rule::QualifiedRule(rule) => transformed.extend(
                    atomicify_qualified_rule(*rule, &options, ctx, None)
                        .into_iter()
                        .map(|replacement| Rule::QualifiedRule(Box::new(replacement))),
                ),
...
fn build_atomic_selector(
    ...
    let mut built: Vec<String> = Vec::with_capacity(selectors.len().max(1));

    let base_selectors: Vec<String> = if selectors.is_empty() {
        vec![String::new()]
    } else {
        selectors.to_vec()
    };
...
fn is_comment_list(_list: &ListOfComponentValues) -> bool {
    !_list.children.is_empty()
        && _list
            .children
            .iter()
            .all(|component| matches!(component, ComponentValue::PreservedToken(_)))
}
