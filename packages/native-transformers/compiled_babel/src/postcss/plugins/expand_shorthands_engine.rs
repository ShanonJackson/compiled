...
pub fn plugin() -> postcss::BuiltPlugin {
    ...
    postcss::plugin("expand-shorthands")
            ...
            for child in rule.nodes() {
                if let Some(decl) = as_declaration(&child) {
                    ...
                    if std::env::var("COMPILED_CLI_TRACE").is_ok() {
                        eprintln!("[expand-shorthands:engine] rule visit prop='{}' value='{}'", prop, decl.value());
                    }
                    ...
                    if expand_shorthand_pairs(&prop.to_lowercase(), &decl.value()).is_some() {
                        ...
                        eprintln!("[expand-shorthands:engine] rule skip prop='{}' (no pairs)", prop);
                    }
                }
            }
            ...
            for child in targets {
                if let Some(decl) = as_declaration(&child) {
                    ...
                    if let Some(pairs) = expand_shorthand_pairs(&prop.to_lowercase(), &value) {
                        ...
                        eprintln!("[expand-shorthands:engine] rule skip prop='{}' (pairs none at apply)", prop);
                    }
                }
            }
        ...
        .decl(|decl, _| {
            ...
            let important = decl.important();
            if std::env::var("COMPILED_CLI_TRACE").is_ok() {
                eprintln!("[expand-shorthands:engine] decl visit prop='{}' value='{}'", prop, value);
            }
            if let Some(pairs) = expand_shorthand_pairs(&prop.to_lowercase(), &value) {
                if std::env::var("COMPILED_CLI_TRACE").is_ok() {
                    eprintln!(
                ...
                eprintln!("[expand-shorthands:engine] decl skip prop='{}' (no pairs)", prop);
            }
            Ok(())
        })
        .at_rule(|at, _| {
            ...
            for child in children.iter() {
                if let Some(decl) = postcss::ast::nodes::as_declaration(child) {
                    let prop = decl.prop();
                    if std::env::var("COMPILED_CLI_TRACE").is_ok() {
                        eprintln!("[expand-shorthands:engine] at-rule visit prop='{}' value='{}'", prop, decl.value());
                    }
                    if expand_shorthand_pairs(&prop.to_lowercase(), &decl.value()).is_some() {
                        targets.push(child.clone());
                    } else if std::env::var("COMPILED_CLI_TRACE").is_ok() {
                        eprintln!("[expand-shorthands:engine] at-rule skip prop='{}' (no pairs)", prop);
                    }
                }
            }
            for child in targets {
                if let Some(decl) = postcss::ast::nodes::as_declaration(&child) {
                    ...
                    if let Some(pairs) = expand_shorthand_pairs(&prop.to_lowercase(), &value) {
                        ...
                        eprintln!("[expand-shorthands:engine] at-rule skip prop='{}' (pairs none at apply)", prop);
                    }
                }
            }
            Ok(())
