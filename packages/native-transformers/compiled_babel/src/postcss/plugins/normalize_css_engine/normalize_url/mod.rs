...
fn is_absolute(url: &str) -> bool {
    ...
    let windows = Regex::new(r"^[a-zA-Z]:\\|
                               ^[a-zA-Z]:/"
    ).unwrap();
...
fn normalize_data_url(url: &str, strip_hash: bool) -> Result<String, ()> {
    ...
    let mut media: Vec<String> = if typ.is_empty() { vec![] } else { typ.split(';').map(|s| s.to_string()).collect() };
    ...
    let mut attrs: Vec<String> = media.into_iter().skip(1).map(|attribute| {
        ...
        if key == "charset" {
            ...
            if value == "us-ascii" { return String::new(); }
        }
        if value.is_empty() { key } else { format!("{}={}", key, value) }
    }).filter(|s| !s.is_empty()).collect();
    if is_base64 { attrs.push("base64".to_string()); }
    if !attrs.is_empty() || (!mime_type.is_empty() && mime_type != "text/plain") {
        attrs.insert(0, mime_type);
    }
    let hash_part = if hash.is_empty() { String::new() } else { format!("#{}", hash) };
    Ok(format!("data:{},{}{}", attrs.join(";"), if is_base64 { data.trim().to_string() } else { data }, hash_part))
}
...
fn path_normalize_like_node(input: &str) -> String {
    ...
    if is_abs { out = format!("/{}", out); }
    if !prefix.is_empty() { out = format!("{}{}", prefix, out); }
    if out.is_empty() { out = if is_abs { "/".to_string() } else { ".".to_string() }; }
    out
}

fn strip_trailing_slash_once(s: &str) -> String { s.trim_end_matches('/').to_string() }

fn strip_www(hostname: &str) -> String {
    // ^www\.(?!www\.) label.tld
    let re = Regex::new(r"^(?i)www\.(?!www\.)").unwrap();
    re.replace(hostname, "").to_string()
}
...
fn normalize_url_impl(url_string_in: &str, opts: &NormalizeOptions) -> Result<String, ()> {
    ...
    if Regex::new(r"(?i)^view-source:").unwrap().is_match(url_string_in) {
        return Err(());
    }

    let mut url_string = url_string_in.trim().to_string();
    let has_relative_protocol = url_string.starts_with("//");
    let is_relative_url = !has_relative_protocol && Regex::new(r"^\.*?/").unwrap().is_match(&url_string);
    if !is_relative_url {
        // Prepend protocol
        let re = Regex::new(r"^(?!(?:\w+:)?//)|^//").unwrap();
        url_string = re.replace(&url_string, &opts.default_protocol).to_string();
    }

    let mut url_obj = Url::parse(&url_string).map_err(|_| ())?;

    if opts.force_http && opts.force_https { return Err(()); }
    if opts.force_http && url_obj.scheme() == "https" { url_obj.set_scheme("http").ok(); }
    if opts.force_https && url_obj.scheme() == "http" { url_obj.set_scheme("https").ok(); }
    ...
    if opts.remove_directory_index {
        ...
        if let Some(last) = path_components.last().cloned() {
            if !last.is_empty() && Regex::new(r"(?i)^index\.[a-z]+$").unwrap().is_match(last) {
                ...
                if !joined.starts_with('/') { joined = format!("/{}", joined); }
    ...
    if let Some(host) = url_obj.host_str() {
        ...
        if opts.strip_www {
            let pat = Regex::new(r"^(?i)www\.(?!www\.)[a-z\-\d]{1,63}\.[a-z.\-\d]{2,63}$").unwrap();
            if pat.is_match(&hostname) {
        ...
        if let Some(p) = port { url_obj.set_port(Some(p)).ok(); }
    }

    // Remove unwanted query parameters
    {
        // Collect into vec
        let mut pairs: Vec<(String, String)> = url_obj.query_pairs().map(|(k, v)| (k.to_string(), v.to_string())).collect();
    ...
    if !opts.remove_single_slash && url_obj.path() == "/" && !old_url_string.ends_with('/') && url_obj.fragment().is_none() {
        out = out.trim_end_matches('/').to_string();
    }
    if (opts.remove_trailing_slash || url_obj.path() == "/") && url_obj.fragment().is_none() && opts.remove_single_slash {
        out = out.trim_end_matches('/').to_string();
    }

    if has_relative_protocol && !opts.normalize_protocol {
        out = Regex::new(r"^http://").unwrap().replace(&out, "//").to_string();
    }
    if opts.strip_protocol {
        out = Regex::new(r"^(?:https?:)?//").unwrap().replace(&out, "").to_string();
    }
