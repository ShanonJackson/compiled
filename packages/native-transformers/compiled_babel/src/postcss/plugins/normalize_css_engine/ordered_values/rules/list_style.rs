...
pub fn normalize(parsed: &vp::ParsedValue) -> String {
    ...
    let mut typ = String::new(); let mut pos = String::new(); let mut img = String::new();
    let mut nodes = parsed.nodes.clone();
    vp::walk(&mut nodes[..], &mut |node| {
        match node {
            vp::Node::Word { value } => {
                let v = value.as_str();
                if list_style_types().contains(v) { typ.push(' '); typ.push_str(v); }
                else if positions.contains(v) { pos.push(' '); pos.push_str(v); }
                else if v == "none" { if typ.split(' ').any(|e| e == "none") { img.push(' '); img.push_str(v); } else { typ.push(' '); typ.push_str(v); } }
                else { typ.push(' '); typ.push_str(v); }
            }
            vp::Node::Function { .. } => { img.push(' '); img.push_str(&vp::stringify(&[node.clone()])); }
            _ => {}
        } true
    }, false);
