            ...
            match &node {
                ...
                vp::Node::Function { value, .. } => {
                    ...
                    if low == "steps" || low == "cubic-bezier" || low == "frames" {
                        if timing.is_empty() { timing.push(node.clone()); timing.push(add_space()); }
                        else { name.push(node.clone()); name.push(add_space()); }
                    } else { name.push(node.clone()); name.push(add_space()); }
                }
                vp::Node::Word { value } => {
                    let low = value.to_lowercase();
                    if let Some(u) = vp::unit::unit(value) {
                        if u.unit == "ms" || u.unit == "s" {
                            if duration.is_empty() { duration.push(node.clone()); duration.push(add_space()); }
                            else if delay.is_empty() { delay.push(node.clone()); delay.push(add_space()); }
                            else { name.push(node.clone()); name.push(add_space()); }
                            continue;
                        }
                        if u.unit.is_empty() {
                            if iteration.is_empty() { iteration.push(node.clone()); iteration.push(add_space()); continue; }
                        }
                    }
                    if matches!(low.as_str(), "ease"|"ease-in"|"ease-in-out"|"ease-out"|"linear"|"step-end"|"step-start") {
                        if timing.is_empty() { timing.push(node.clone()); timing.push(add_space()); } else { name.push(node.clone()); name.push(add_space()); }
                    } else if matches!(low.as_str(), "normal"|"reverse"|"alternate"|"alternate-reverse") {
                        if direction.is_empty() { direction.push(node.clone()); direction.push(add_space()); }
                ...
                _ => { name.push(node.clone()); name.push(add_space()); }
            }
        }
