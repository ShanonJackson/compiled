...
fn stringify_node(node: &Node, precision: usize) -> String {
    match node {
        ...
        Node::Op(OpKind { op, left, right }) => {
            let wrap_left = matches!(left.as_ref(), Node::Op(OpKind { op: l, .. }) if op_prec(*op) < op_prec(*l));
            let wrap_right = matches!(right.as_ref(), Node::Op(OpKind { op: r, .. }) if op_prec(*op) < op_prec(*r));
...
pub fn reduce_calc_value(value: &str, precision: usize) -> Option<(String, bool)> {
    ...
    for n in parsed.nodes.iter_mut() {
        ...
        } = n
        {
            ...
            if let Some(ast) = parse_calc_expression(&inner) {
                let reduced = reduce_node(ast, precision);
                match reduced {
                    Node::Value(ValueKind { num, unit }) => {
                        let mut v = fmt_number(num, precision);
                        if let Some(u) = unit {
                            v.push_str(&u);
                        }
                        if v != inner {
                    ...
                    other => {
                        let expr = stringify_ast(&other, precision);
                        let wrapped = format!("{}({})", name, expr);
                        if wrapped != format!("{}({})", name, inner) {
                            changed = true;
                        }
                        unresolved = true;
    ...
    if !changed {
        return Some((value.to_string(), unresolved));
    }

    Some((vp::stringify(&parsed.nodes), unresolved))
}
            ...
            let process_decl = |decl: postcss::ast::nodes::Declaration| {
                ...
                for n in parsed.nodes.iter_mut() {
                    ...
                    } = n
                    {
                        let name_l = name.to_ascii_lowercase();
                        let is_calc =
                            name_l == "calc" || name_l == "-webkit-calc" || name_l == "-moz-calc";
                        if !is_calc {
                            continue;
                        }
