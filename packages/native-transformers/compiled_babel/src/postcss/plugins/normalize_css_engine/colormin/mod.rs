...
pub struct ColorminOptions {
    pub transparent: bool,
    pub alpha_hex: bool,
    pub name: bool,
}

pub fn add_plugin_defaults(browsers: &[String]) -> ColorminOptions {
    ...
    let alpha_hex =
        if let Ok(supported) = oxc_browserslist::resolve(&["supports css-rrggbbaa"], &opts) {
...
pub fn default_options_with_browsers() -> (ColorminOptions, Vec<String>) {
...
fn minify_color(input: &str, options: &ColorminOptions) -> String {
    ...
    if let Ok(color) = parsed {
        ...
        if a >= 1.0 {
            ...
            let a_str = number_short(a as f64);
            candidates.push(
                format!("hsla({},{}% ,{}%,{})", h_str, s_str, l_str, a_str).replace(" %", "%"),
            );
        }
...
pub fn transform_value(value: &str, options: &ColorminOptions) -> String {
...
pub fn resolve_browsers() -> Vec<String> {
    let opts = oxc_browserslist::Opts::default();
