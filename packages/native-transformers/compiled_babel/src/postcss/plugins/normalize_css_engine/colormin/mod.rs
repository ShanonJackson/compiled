...
static NAME_TO_HEX: Lazy<HashMap<&'static str, &'static str>> = Lazy::new(|| {
    ...
    let mut m = HashMap::new();
    for (name, hex) in pairs.iter() { m.insert(*name, *hex); }
    m
});
...
pub(crate) struct ColorminOptions { transparent: bool, alpha_hex: bool, name: bool }
...
fn minify_color(input: &str, options: &ColorminOptions) -> String {
    ...
    if let Ok(color) = parsed {
        ...
        if a >= 1.0 || options.alpha_hex {
            let base = if a >= 1.0 { format!("#{:02x}{:02x}{:02x}", r, g, b) }
                       else { hex_string_from_rgba(r, g, b, a as f32) };
            ...
            if a >= 1.0 {
                ...
                if let Some(short) = short_hex_candidate(&base, alpha) { candidates.push(short); }
        ...
        else { let a_str = number_short(a as f64); candidates.push(format!("hsla({},{}% ,{}%,{})", h_str, s_str, l_str, a_str).replace(" %", "%")); }
    ...
    if let Some(hex) = NAME_TO_HEX.get(lower.as_str()) {
        ...
        if short.len() < input.len() { return short; }
...
fn walk(parent: &mut vp::ParsedValue, cb: &mut dyn FnMut(&mut vp::Node, usize) -> (bool, Option<vp::Node>)) {
    let mut i = 0usize;
    while i < parent.nodes.len() {
        ...
        if let Some(to_insert) = insert_after { parent.nodes.insert(i + 1, to_insert); i += 1; }
        if let vp::Node::Function { nodes, .. } = &mut parent.nodes[i] {
            if bubble {
                let mut inner = vp::ParsedValue { nodes: nodes.clone() };
                walk(&mut inner, cb);
                *nodes = inner.nodes;
            }
        }
                ...
                if Regex::new(r"^(?i)(rgb|hsl)a?$").unwrap().is_match(value) {
                    ...
                    *node = vp::Node::Word { value: newv.clone() };
                    if newv.to_lowercase() != original.to_lowercase() {
                        return (true, Some(vp::Node::Space{ value: " ".to_string()}));
                    }
                } else if is_math_function(node) { return (false, None); }
            }
...
pub fn plugin() -> pc::BuiltPlugin {
    ...
    pc::plugin("postcss-colormin")
        .decl(move |decl, _| {
            ...
            let key = format!(
                "{:?}",
                (&value, options.transparent, options.alpha_hex, options.name, &browsers)
            );
