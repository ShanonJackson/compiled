...
static HEX_TO_NAME: Lazy<HashMap<&'static str, &'static str>> = Lazy::new(|| {
    ...
    for (name, hex) in pairs.iter() { m.insert(*hex, *name); }
    m
});

fn is_math_function(node: &vp::Node) -> bool {
    match node { vp::Node::Function { value, .. } => matches!(value.to_lowercase().as_str(), "calc"|"min"|"max"|"clamp"), _ => false }
}

#[derive(Clone, Copy, Debug)]
struct ColorminOptions { transparent: bool, alpha_hex: bool, name: bool }

fn add_plugin_defaults() -> ColorminOptions {
    ...
    // - name: true
    ColorminOptions { transparent: true, alpha_hex: false, name: true }
}

fn number_short(n: f32) -> String {
    // If between 0 and 1, emit like .5 instead of 0.5
    if n > 0.0 && n < 1.0 {
        let s = ryu_js::Buffer::new().format_finite(n);
...
fn short_hex_candidate(base_hex: &str, alpha: f32) -> Option<String> {
    ...
    let (s,o,u,l,p,f,g,v) = (chars[1],chars[2],chars[3],chars[4],chars[5],chars[6], *chars.get(7).unwrap_or(&'f'), *chars.get(8).unwrap_or(&'f'));
    if alpha > 0.0 && alpha < 1.0 {
        // JS plugin rejects alpha hex for fractional alpha (n=0)
        return None;
    }
    if s == o && u == l && p == f {
        if alpha == 1.0 { return Some(format!("#{}{}{}", s, u, p)); }
        if alpha == 0.0 && g == v { return Some(format!("#{}{}{}{}", s, u, p, g)); }
    }
...
fn rgba_to_hsl(r: u8, g: u8, b: u8) -> (f32, f32, f32) {
    ...
    let mut h; let l = (max + min) / 2.0;
    let d = max - min;
    let s = if d == 0.0 { 0.0 } else { d / (1.0 - (2.0*l - 1.0).abs()) };
    if d == 0.0 { h = 0.0; }
    else if max == r { h = ((g - b) / d) % 6.0; }
...
fn minify_color(input: &str, options: &ColorminOptions) -> String {
    // Try parsing with csscolorparser (supports named, hex, rgb/rgba, hsl/hsla, etc.)
    let parsed = csscolorparser::Color::from_str(input);
    if let Ok(color) = parsed {
        let (r_f, g_f, b_f, a_f) = (color.r, color.g, color.b, color.a);
        let (r, g, b) = ((r_f * 255.0).round().clamp(0.0, 255.0) as u8,
        ...
        // hex first when allowed
        if a >= 1.0 || options.alpha_hex {
            let base = if a >= 1.0 { format!("#{:02x}{:02x}{:02x}", r, g, b) }
                       else { hex_string_from_rgba(r, g, b, a) };
            if let Some(short) = short_hex_candidate(&base, a.min(1.0).max(0.0)) { candidates.push(short); }
        }
        ...
        let h_str = number_short(h);
        let s_str = number_short(s);
        let l_str = number_short(l);
        if a >= 1.0 { candidates.push(format!("hsl({},{}% ,{}%)", h_str, s_str, l_str).replace(" %", "%")); }
        else { let a_str = number_short(a); candidates.push(format!("hsla({},{}% ,{}%,{})", h_str, s_str, l_str, a_str).replace(" %", "%")); }

        // transparent
        if options.transparent && r == 0 && g == 0 && b == 0 && a == 0.0 { candidates.push("transparent".to_string()); }

        // name (only if opaque)
        if options.name && a >= 1.0 {
            let hex6 = format!("#{:02x}{:02x}{:02x}", r, g, b);
            if let Some(name) = HEX_TO_NAME.get(hex6.as_str()) { candidates.push((*name).to_string()); }
        }
...
fn walk(parent: &mut vp::ParsedValue, cb: &mut dyn FnMut(&mut vp::Node, usize, &mut vp::ParsedValue) -> bool) {
    for i in 0..parent.nodes.len() {
        let bubble = cb(&mut parent.nodes[i], i, parent);
        if let vp::Node::Function { nodes, .. } = &mut parent.nodes[i] { if bubble { let mut inner = vp::ParsedValue { nodes: nodes.clone() }; walk(&mut inner, cb); *nodes = inner.nodes; } }
    }
}

fn transform(value: &str, options: &ColorminOptions) -> String {
    let mut parsed = vp::parse(value);
    walk(&mut parsed, &mut |node, index, parent| {
        match node {
            vp::Node::Function { value, nodes, .. } => {
                if Regex::new(r"^(?i)(rgb|hsl)a?$").unwrap().is_match(value) {
                    ...
                    *node = vp::Node::Word { value: newv.clone() };
                    if newv.to_lowercase() != original.to_lowercase() {
                        if let Some(next) = parent.nodes.get(index+1) {
                            match next { vp::Node::Word { .. } | vp::Node::Function { .. } => parent.nodes.insert(index+1, vp::Node::Space{ value: " ".to_string()}), _=>{} }
                        }
                    }
                } else if is_math_function(node) { return false; }
            }
            vp::Node::Word { value } => { *value = minify_color(value, options); }
            _ => {}
        }
        true
    });
...
pub fn plugin() -> pc::BuiltPlugin {
    pc::plugin("postcss-colormin")
        .prepare(|_result| {
            ...
            if has_ie8_9 { options.transparent = false; }
            ...
            pc::PreparedCallbacks::default().once_exit(move |css, _| {
                css.walk_decls(|decl, _| {
                    if Regex::new(r"(?i)^(composes|font|src$|filter|-webkit-tap-highlight-color)").unwrap().is_match(&decl.prop()) { return true; }
                    let value = decl.value(); if value.is_empty() { return true; }
                    let key = format!("{:?}", (&value, options.transparent, options.alpha_hex, options.name, &browsers));
                    if let Some(v) = cache.lock().unwrap().get(&key).cloned() { decl.set_value(v); return true; }
                    let newv = transform(&value, &options);
                    decl.set_value(newv.clone()); cache.lock().unwrap().insert(key, newv);
                    true
                });
                Ok(())
            })
