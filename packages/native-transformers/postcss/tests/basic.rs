...
use postcss::{
  ast::nodes::{
    as_comment, as_declaration, as_rule, Declaration, Root as AstRoot, RootLike, Rule as AstRule,
  },
  ast::NodeAccess,
...
impl Plugin for UppercasePlugin {
  ...
  fn visit_rule(&self, rule: &AstRule, _result: &mut PostcssResult) -> Result<(), ProcessorError> {
...
impl Plugin for VisitorRecorder {
  ...
  fn visit_root(&self, _root: &AstRoot, _result: &mut PostcssResult) -> Result<(), ProcessorError> {
...
fn processor_uses_custom_stringifier_when_provided() {
  ...
  let options = ProcessOptions::new().stringifier(
    move |root_like: &postcss::ast::nodes::RootLike,
          builder: &mut dyn FnMut(&str, Option<&postcss::ast::NodeRef>, Option<&'static str>)| {
